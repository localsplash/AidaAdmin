/**
 * Read-only view of OfficePulseAidaIntegration's `aidacalls_db` runtime
 ...
 * on `aidacalls_db.*` and nothing else — so read-only is a property of
  ...
  private async select(sql: string, params: Param[] = []): Promise<Row[]> {
    try {
      ...
      throw new RuntimeDbError(
        `aidacalls_db read failed (${(err as { code?: string }).code ?? 'unknown'})`,
      );
    }
  }
