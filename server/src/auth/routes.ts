...
import { IdClientError, type IdClient } from '../id/client.js';
import type { LoginStateStore } from './state-store.js';
import type { SessionStore } from './session-store.js';
import type { TenantUserDirectory } from './tenant-directory.js';
...
export function authRoutes(config: AppConfig, logger: Logger, deps: AuthDeps): Router {
  const router = Router();
  ...
  router.get('/api/auth/callback', async (req, res) => {
    ...
    if (!state || !code || state !== stateCookie || !deps.stateStore.consume(state)) {
      failLogin(res, 'error');
      return;
    }
    ...
    try {
      ...
      if (err instanceof IdClientError) {
        logger.warn({ status: err.status, correlationId: req.correlationId }, 'code redeem failed');
        failLogin(res, 'error');
        return;
      }
      throw err;
    ...
    res.cookie(SESSION_COOKIE, session.sid, cookieOptions(config, true));
    res.redirect('/');
  });
  ...
  router.post('/api/auth/logout', (req, res) => {
    ...
    if (sid) deps.sessionStore.revoke(sid);
