...
test('build identity is stable, Pacific, revision-specific, and marks uncommitted changes', () => {
  ...
  try {
    ...
    execFileSync('git', ['commit', '-m', 'fixture'], {
      cwd,
      env: { ...env, GIT_AUTHOR_DATE: timestamp, GIT_COMMITTER_DATE: timestamp },
      stdio: 'pipe',
    });
    const clean = build();
    assert.deepEqual(clean, {
      version: '2026.1.1.14.4',
      revision: git('rev-parse', 'HEAD'),
      sourceUpdatedAt: '2026-01-01T14:04:59-08:00',
    ...
    git('add', 'new-source');
    assert.equal(build().dirty, true);
    execFileSync('git', ['commit', '-m', 'same minute'], {
      cwd,
      env: { ...env, GIT_AUTHOR_DATE: timestamp, GIT_COMMITTER_DATE: timestamp },
      stdio: 'pipe',
    });
    ...
    assert.deepEqual(
      build({ BUILD_REVISION: clean.revision, SOURCE_DATE_EPOCH: epoch, BUILD_DIRTY: 'false' }),
      clean,
    );
    for (const [utc, version, local] of [
      ...
      const info = build({
        BUILD_REVISION: clean.revision,
        SOURCE_DATE_EPOCH: String(Date.parse(utc) / 1000),
        BUILD_DIRTY: 'false',
        TZ: 'UTC',
      });
    ...
    assert.throws(() =>
      build({ BUILD_REVISION: clean.revision, SOURCE_DATE_EPOCH: 'invalid', BUILD_DIRTY: 'false' }),
    );
  } finally {
...
test('runtime defaults to Pacific and honors an explicit timezone', () => {
  const modulePath = fileURLToPath(new URL('../server/src/timezone.ts', import.meta.url));
  for (const [configured, expected] of [
    ...
    const output = execFileSync(
      ...
      [
        ...
        'tsx',
        '-e',
        `require(${JSON.stringify(modulePath)}); console.log(process.env.TZ)`,
      ],
      {
        env: { ...process.env, TZ: configured },
        encoding: 'utf8',
      },
    ).trim();
