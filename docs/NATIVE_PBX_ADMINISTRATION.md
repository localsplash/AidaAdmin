...
| `/did-routes`                                     | GET        | `/dids?iTenantId=N` plus enabled Identity voice-number intersection          |
...
Legacy extension/ring-group/DID routes, payload builders and retry controls are removed. Runtime provisioning/fallback views and SQL accessors are retired. The former NocoDB native tables and fields remain available only for deliberate export/rollback inspection; their repositories/schema definitions are not used by the native PBX routes. No data deletion or migration into native routing is automatic.

Before retiring `aida_tbl_Extension`, `aida_tbl_RingGroup`, `aida_tbl_RingGroupMember`, `aida_tbl_DidRoute`, and obsolete handset/projection fields: export records and attachments, retain original IDs/tenant mappings, inventory other consumers, compare actual native Asterisk state, and review retention/removal separately. Preserve `aida_tbl_AssistantProfile`, tenant business metadata, appearance, Identity assignments and audit history. Legacy records must never be silently replayed into PBX configuration.
